...
const uninstallScriptPath = path.join(__dirname, 'uninstall_injDetPlugin.js');
const tempBlacklistFilePath = path.join(__dirname, 'blacklists', 'tempBlacklist.json');
const permBlacklistFilePath = path.join(__dirname, 'blacklists', 'permBlacklist.json');
const attackCountFilePath = path.join(__dirname, 'logs', 'attackCount.json');
...
function uninstallPlugin() {
    ...
    // 删除 tempBlacklist.json
    if (fs.existsSync(tempBlacklistFilePath)) {
        fs.unlinkSync(tempBlacklistFilePath);
        console.log(\`已删除文件: \${tempBlacklistFilePath}\`);
    }

    // 删除 permBlacklist.json
    if (fs.existsSync(permBlacklistFilePath)) {
        fs.unlinkSync(permBlacklistFilePath);
        console.log(\`已删除文件: \${permBlacklistFilePath}\`);
    }

    // 删除 attackCount.json
    if (fs.existsSync(attackCountFilePath)) {
        fs.unlinkSync(attackCountFilePath);
        console.log(\`已删除文件: \${attackCountFilePath}\`);
    }
    ...
    // 检查并删除 blacklists 目录（如果为空）
    if (fs.existsSync(blacklistsDir) && isDirectoryEmpty(blacklistsDir)) {
        fs.rmdirSync(blacklistsDir);
        console.log(\`已删除空目录: \${blacklistsDir}\`);
    }
